...
* **[chatbot.js](file:///c:/Users/PC/Desktop/subject/AUBE/chatbot.js)**
  * **수정 이유**: 메인 웹페이지 언어 셀렉터 변경 시 챗봇 언어 동조 연동 및 챗봇 내 연락처 카드의 WeChat QR 터치 트래킹 연동.
  * **수정 내용**: `window.changeChatbotLanguage` 핸들러 외부에 바인딩 노출 및 WeChat QR 클릭 리스너 신설.
  * **수정 후 기대 효과**: 매끄러운 다국어 사용자 동조 연동 및 정확한 WeChat 전환 지표 집계 가능.
* **[index.html](file:///c:/Users/PC/Desktop/subject/AUBE/index.html)**
  * **수정 이유**: 다국어 싱크 제어 및 메인 WeChat QR 이미지 클릭 시 트래킹 추가.
  * **수정 내용**: `changePageLang()` 함수 내에 챗봇 언어 동기화 호출 삽입 및 WeChat QR 클릭 이벤트 전송 리스너 결합.
  * **수정 후 기대 효과**: 일체감 있는 다국어 UX 및 메인 WeChat 전환 지표 보강.
* **[notice.html](file:///c:/Users/PC/Desktop/subject/AUBE/notice.html)**
  * **수정 이유**: 지도 링크 클릭 시 WeChat 카운트가 올라가는 오분석 현상 제거 및 다국어 싱크, 실제 WeChat QR 클릭 추적 탑재.
  * **수정 내용**: `setLang()` 함수 내에 챗봇 언어 동기화 삽입, 지도 이동 트래킹을 `map_click`으로 변경, 실제 WeChat QR 이미지 터치 시 `wechat_click`이 전송되도록 이벤트 매칭 교정.
  * **수정 후 기대 효과**: GA4 데이터 분석의 신뢰성 정복 및 정확한 전환 모니터링 가능.
...
발견된 오트래킹 결함 및 JSON 안정성 개선 작업이 로컬 저장소 상에 완벽하게 커밋 완료되었으므로, 호스팅 서버(Vercel 등)로 즉시 배포하여 사용해도 되는 수준의 안전한 상태입니다.
