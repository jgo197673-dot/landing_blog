...
## 10. 긴급 트러블슈팅 이력 (2026-06-06)

### 🚨 챗봇 질문 시 "죄송합니다. 일시적인 오류..." 메시지만 상시 출력되는 문제
* **발생 상황**: 로컬 실행 환경에서 어떠한 질문을 해도 지속해서 일시적 오류 문구만 반복 노출됨.
* **원인 추적**:
  1. **에러 발생 코드 위치**: [chatbot.js](file:///c:/Users/PC/Desktop/subject/AUBE/chatbot.js)의 `sendMessageText()` 내 `catch (err)` 예외 처리 블록.
  2. **트리거 조건**: `fetch('/api/chat')` 요청 시 `response.ok`가 `false`로 인식되어 `Error`가 던져지는 경우.
  3. **실제 로컬 실행 에러 원인 (Critical)**: 로컬 실행 환경에서 `.env.local`에 기입된 `GEMINI_API_KEY`가 Google AI Studio의 유효한 키가 아니라 `your_gemini_api_key_here`라는 가짜 플레이스홀더 키 상태로 연동되어 작동하고 있었습니다. 이에 따라 서버리스 함수인 `api/chat.js`가 Gemini API 서버로부터 `400 Bad Request` 에러를 반환받았고, `api/chat.js`는 상태코드 `502`와 함께 폴백 JSON 데이터를 반환했습니다.
  4. **클라이언트 측 처리 한계**: 기존 클라이언트 코드(`chatbot.js`)는 응답 코드가 `200` 계열이 아닌 `502`인 경우, 백엔드가 반환한 정상적인 폴백 메시지 JSON 데이터가 있어도 이를 파싱하지 못하고 무조건 `catch` 블록으로 떨어져 범용 에러 문구(`"죄송합니다. 일시적인 오류..."`)만 노출했습니다.
* **조치 사항**:
  * **[chatbot.js](file:///c:/Users/PC/Desktop/subject/AUBE/chatbot.js) (클라이언트 개선)**: `!response.ok` 상황이더라도 서버가 반환한 응답 바디가 유효한 JSON이고, 그 안에 대체 답변(`data.answer` 또는 `data.reply`)이 포함되어 있다면 이를 파싱하여 정상 화면에 출력하고 Contact Card까지 부드럽게 렌더링을 연동하도록 `fetch` 처리부를 개선했습니다.
  * **사용자 가이드**: 로컬 테스트 진행을 위해 사용자의 실제 구글 AI 스튜디오 API Key를 `.env.local`의 `GEMINI_API_KEY` 값으로 교체하여 동작을 확인해야 함을 공유합니다. (키가 올바르게 주입되면 502/500 에러를 타지 않고 200 정상 답변이 생성됩니다.)
